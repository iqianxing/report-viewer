...
module.exports = function (server, program) {
  ...
  io.on("connection", function (socket) {
    ...
    if (program.args.length > 0) {
      ...
      if (restart) {
        spec = socket.request._query["spec"] || program.spec;
        project = socket.request._query["project"] || program.project;
  ...
  if (program.args.length === 0) {
    ...
    restart = function (spec, project, context, roomId) {
      ...
      var cmd = workspace.get(project, spec, context);
      child = spawn(cmd.sh, cmd.args, {
        cwd: cmd.cwd,
        env: cmd.env,
      });
