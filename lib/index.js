...
(function() {
  ...
  module.exports = function(program) {
    var addOptionsToClient, child, close, currentConsole, dataManager, ended, loaded, port, restart, server, site;
    currentConsole = [];
    ...
    if (program.opener) {
      ...
      var child_ns = io.of("/");
      child_ns.to(socket.id).emit("reload");
    }
    if (site.action) {
      site.action.on("reload", function() {
        ...
        var child_ns = io.of("/");
        return child_ns.to(socket.id).emit("reload");        
      });
    }
